...
function writable<T>(map: Y.Map<T>) {
  let value: Map<string, T> = new Map(Object.entries(map.toJSON()))
  let subs = []
  ...
  const set = (newValue: Map<string, T>) => {
    // "setting" a YMap is kind of a worst-case usage, but we'll do it
    // in order to comply with the Svelte expectation of a 'set' function

    // Delete everything
    map.forEach((_item, key) => map.delete(key))

    // Set everything
    for (let [key, value] of newValue) {
      map.set(key, value)
    }
  }
